...
public class EditPublish implements IObjectActionDelegate {
		...
		if (editorPart instanceof ITextEditor) {
			...
			if (editorPart instanceof ITextEditor) {
				...
				try {
					...
					codeHtml = "<ol start='1' style='margin-left: 0;'>" +  codeHtml.replaceAll("<p class=default>", "<li style=\"padding-left: 10px; border-left: 1px solid #D1D7DC; margin: 0 0 0 38px;\">").replaceAll("</p>", "</li>") + "</ol>";
					...
					BlogEditor be = new BlogEditor(PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell(), blogFile);
					be.open();
					/*
	...
	public void selectionChanged(IAction action, ISelection selection) {
		System.out.println("in method selectionChanged!");
	}

	public void setActivePart(IAction action, IWorkbenchPart targetPart) {
		shell = targetPart.getSite().getShell();
		System.out.println("in method setActivePart!");
	}
}
